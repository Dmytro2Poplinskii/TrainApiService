...
class TrainViewSetTest(APITestCase):
    ...
    def test_create_train(self):
        ...
        response = self.client.post(
            "/api/v1/trains/", new_train_data, format="multipart"
        )
        print(response.data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(
            response.data["places_in_carriage"], new_train_data["places_in_carriage"]
        )
...
class RouteViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.route2 = Route.objects.create(
            ...
            destination=Station.objects.create(
                name="Station 3", latitude=3.0, longitude=3.0
            ),
        )
    ...
    def test_retrieve_route(self):
        ...
        self.assertEqual(
            response.data["destination"]["name"], self.route1.destination.name
        )
...
class JourneyViewSetTest(APITestCase):
    ...
    def test_retrieve_journey(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data["route"]["source"]["name"], self.journey.route.source.name
        )
        self.assertEqual(
            response.data["route"]["destination"]["name"],
            self.journey.route.destination.name,
        )
